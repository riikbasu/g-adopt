...
# +
solutions_vtk = VTKFile("solutions_test.pvd")
functional_values = []
...
# See if Hessian is hit
def record_pre_hess(*args):
    log(f"Hessian calculation started")

def record_post_hess(*args):
    log(f"Hessian calculation finished")
...
def callback():
    ...
    reduced_functional.eval_cb_post = record_functional_values
    ...
    if MPI.COMM_WORLD.Get_rank() == 0:        
        with open("functional_test.txt", "a") as f:
            if functional_values:            
