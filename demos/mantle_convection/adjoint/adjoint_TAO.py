...
# ## Using TAO trust region optimiser with L-BFGS
...
solver = TAOSolver(minimisation_problem, {
    ...
    # "tao_init_type": "constant",         # Initial point type ('constant'|'given'|'random'); default: 'given'
    # "tao_view": "",                      # Print solver options at startup; default: False
    # "tao_view_solution": "",             # Print final solution vector; default: False
    "tao_type": "bqnkls",                 # Bounded Limited-memory Variable Metric (L-BFGS + bounds)
                                         # Default: nls (nonlinear least-squares)
    # --- Monitoring and Stopping Criteria ---
    # "tao_monitor": "",                     # Print iteration progress; default: False
    # "tao_monitor_all": "",               # Print iteration progress; default: False
    "tao_monitor_globalization": "",
    # "tao_converged_reason": "",            # Print convergence reason; default: False
    "tao_ls_monitor":"",
    "snes_linesearch_monitor":"",
    "tao_gatol": 1.0e-8,                   # Gradient absolute tolerance; default: 1e-8
    "tao_grtol": 0.0,                      # Relative gradient tolerance; default: 1e-8
    "tao_gttol": 0.0,                      # Step-size tolerance; default: 1e-12
    "tao_max_it": 20,                      # Max iterations; default: 200
    # "log_view": "",                     # PETSc option to print a performance summary
    # "petsc_log_view": "",              # Alternative prefix if the above fails

    # # --- L-BFGS Memory and Scaling Options ---
    "tao_lmm_vectors": 10,               # Number of (s,y) pairs stored in memory; default: 5
    "tao_lmm_scale_type": "scalar",      # Scaling type for initial Hessian ('none'|'scalar'|'diagonal'); default: 'scalar'
    # "tao_lmm_limit_type": "average",     # Limit memory strategy ('none'|'average'); default: 'none'
    # "tao_lmm_bfgs": True,                # Use BFGS updates (True) or DFP (False); default: True
    # "tao_lmm_alpha": 1.0,                # Step damping factor; default: 1.0
    # "tao_bfgs_rescale": True,            # Rescale if curvature violated; default: True

    # # --- Bound Constraints and Active Set ---
    # "tao_blmvm_as_type": "grad",         # Active-set strategy ('grad'|'proj'); default: 'grad'
    # "tao_bounded_type": "clip",          # How to enforce bounds ('clip'|'project'); default: 'clip'
    # "tao_bmvm_eps": 1.0e-8,              # Feasibility tolerance for bounds; default: 1e-8

    # # --- Line Search Configuration ---
    # "tao_ls_type": "more-thuente",       # Line search method ('more-thuente'|'armijo'|'gpcg'); default: 'more-thuente'
    # "tao_ls_ftol": 1e-4,                 # Sufficient decrease condition (Armijo parameter); default: 1e-4
    # "tao_ls_gtol": 0.9,                  # Curvature condition tolerance; default: 0.9
    # "tao_ls_stepmin": 1e-10,             # Minimum step length; default: 1e-20
    # "tao_ls_stepmax": 1e2,               # Maximum step length; default: 1e+10
    # "tao_ls_rtol": 1e-10,                # Relative line search tolerance; default: 1e-10
    # "tao_max_funcs": 50000,              # Maximum number of objective evaluations; default: 10000
})
...
def monitor(tao):
    ...
    print(f"TAO Iteration: {its} | Functional: {f} | Function evaluations: {counter_func} | Gradient evaluations: {counter_grad} | Hessian evaluations: {counter_hess} | Gradient Norm: {res} | C-Norm: {cnorm} | Step Size: {step}")
    # Write functional and misfit values to a file (appending to avoid overwriting)
    if MPI.COMM_WORLD.Get_rank() == 0:        
        with open(functional_file, "a") as file:
            file.write(f"TAO Iteration: {its} | Functional: {f} | Function evaluations: {counter_func} | Gradient evaluations: {counter_grad} | Hessian evaluations: {counter_hess} | Gradient Norm: {res} | C-Norm: {cnorm} | Step Size: {step} \n")
...
# solver = TAOSolver(minimisation_problem, 
#                    {
#                        "tao_type": "bqnls",                 # Bounded (L-BFGS + bounds)
#                        "tao_monitor_globalization": "",
#                    },
#                    comm = comm,
#                   )
...
# # def monitor(tao):
# #     # print(f"TAO Iteration: {its:02d} | Functional: {f:.6e} | Gradient Norm: {res:.3e} | Step Size: {step:.3e}")
# #     its = tao.getIterationNumber()
# #     f = tao.getObjectiveValue()
# #     print(f"TAO Iteration: {its:02d} | Functional: {f:.6e}")
...
#     print(f"TAO Iteration: {its} | Functional: {f} | Function evaluations: {nfev} | Gradient Norm: {res} | C-Norm: {cnorm} | Step Size: {step}")
...
# # define the reduced objective functional
# J = assemble((u - uref)**2 * dx)
# reduced_functional = ReducedFunctional(J, Control(source))
