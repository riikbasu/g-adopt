...
functional_file = "functional_scipy.txt"
solutions_vtk = VTKFile("solutions_scipy.pvd")
...
def callback(intermediate_result):
    ...
    if MPI.COMM_WORLD.Get_rank() == 0:
        with open(functional_file, "a") as file:
            file.write(f"Iteration: {its} | Functional: {intermediate_result.fun} | Function evaluations: {sf.nfev} | Gradient evaluations: {sf.ngev} | Initial Misfit: {initial_misfit} | Final Misfit: {final_misfit} \n")
...
sol = minimize(ReducedFunctionalNumPy(rf), method = 'L-BFGS-B', bounds=None, tol=1e-12, callback=callback, options={"disp": True, "maxiter": 2})
# -
