...
norm_smoothing = assemble(dot(grad(Taverage), grad(Taverage)) * dx)
...
minimisation_parameters["Step"]["Line Search"] = {
    ...
    # "Sufficient Decrease Tolerance": 1e-20,
    # "Curvature Condition": {"General Parameter":1e-15}
...
solutions_vtk = VTKFile("solutions_NK_focussed_tests_alphad_1_alphas_plus_1.pvd")
...
class StatusTest(ROL.StatusTest):
    def check(self, status):
        ...
        if MPI.COMM_WORLD.Get_rank() == 0:        
            with open("functional_NK_focussed_tests_alphad_1_alphas_2.txt", "a") as f:
...
# filenames = ['functional_NK_tests_1.txt', 'functional_NK_focussed_tests.txt']
# labels = ['default', 'test']
# plot_type = 'Initial Misfit'  # Define one of ['Functional values', 'Initial Misfit', 'Final Misfit']
