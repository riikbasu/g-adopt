...
def model(level, nn, do_write=False):
    ...
    # compute ns analytical and error (note we are using the same space as pressure)
    nsdg = Function(Q1DG).interpolate(ns, Q1DG)
    ...
    ns_error = Function(Q1DG, name="PressureError").assign(nsdg - p_anal)
    ...
    if do_write:
        ...
        ns_file = VTKFile("ns_pressure_{}.pvd".format(level))

        # Write output:
        u_file.write(u_, u_anal, u_error)
        p_file.write(p_, p_anal, p_error)
        ns_file.write(ns, ns_anal, ns_error)
