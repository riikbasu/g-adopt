...
def model(level, nn, do_write=False):
    """The discontinuous pressure delta-function initial condition, cylindrical domain, zero-slip boundary condition model.
    ...
    ns_solver = BoundaryNormalStressSolver(stokes_solver, boundary.top, solver_parameters=_project_solver_parameters)
    ns_ = ns_solver.solve()
    ...
    if do_write:
        ...
        ns_file = VTKFile("zs_normalstress_{}.pvd".format(level))
